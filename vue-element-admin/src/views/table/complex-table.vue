<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-input v-model="listQuery.title" v-model.trim="listQuery.title" placeholder="标题" style="width: 200px;" class="filter-item" clearable @keyup.enter.native="handleFilter" />
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="时间" prop="timestamp">
          <el-date-picker v-model="temp.timestamp" type="datetime" placeholder="请选择一个时间" />
        </el-form-item>
    ...
    <!--阅读量详情页-->
    <el-dialog :visible.sync="dialogPvVisible" title="阅读量详情">
      <el-table :data="pvData" border fit highlight-current-row style="width: 100%">
        <el-table-column prop="key" label="渠道" />
        <el-table-column prop="pv" label="数量" />
      </el-table>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="dialogPvVisible = false">返回</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
       ...
       */
      rules: {
        type: [{ required: true, message: '类型必输', trigger: 'change' }],
        timestamp: [{ type: 'date', required: true, message: '时间必输', trigger: 'change' }],
        title: [{ required: true, message: '标题必输', trigger: 'blur' }]
      },
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          createArticle(this.temp).then(() => {
            console.info('提交数据' + JSON.stringify(this.temp))
